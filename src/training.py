...
class CCA(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        """x: (batch, dim, seq)"""
        h = x.mean(dim=-1)
        h = self.ac(self.linear(h))
        return x * h.unsqueeze(-1)


class TransformerCNNLayer(nn.Module):
    def __init__(self, dim=64, num_heads=4, mlp_ratio=4.0):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x):
        batch, seqt, seqf, dim = x.shape

        xf = self.norm(x)
        xf = rearrange(xf, "b t f d -> (b t) f d")
        xf, _ = self.attn(xf, xf, xf)
        xf = rearrange(xf, "(b t) f d -> b t f d", b=batch, t=seqt)

        xt = rearrange(x, "b t f d -> (b f) d t")
        xt = self.cnn(xt)
        xt = rearrange(xt, "(b f) d t -> b t f d", b=batch, f=seqf)
        x = x + xf + xt

        h = rearrange(x, "b t f d -> (b t f) d")
        h = self.mlp(h)
        h = rearrange(h, "(b t f) d -> b t f d", b=batch, t=seqt, f=seqf)
        x = x + h
...
class MusicTranscription(nn.Module):
    """
    Transformer in frequency + CNN (w. CCA) in time
    """
    def __init__(self, dim=96, n_heads=3, depth=4):
        super().__init__()
        self.encoder = nn.Sequential(  # (time, freq) -> (time//12, freq//24)
            nn.Conv2d(1, dim//2, kernel_size=5, stride=3),
            nn.GroupNorm(num_groups=dim//16, num_channels=dim//2),
            nn.GELU(),
        ...
        self.model = nn.Sequential(*(
            TransformerCNNLayer(dim=dim, num_heads=n_heads, mlp_ratio=2.0)
            for _ in range(depth)
        ))
        self.decoder = nn.Sequential(
            nn.GroupNorm(num_groups=dim//8, num_channels=dim),
            nn.ConvTranspose2d(dim, 1, kernel_size=(12, 3), stride=(12, 3)),
        )

        self.pos: Tensor
        self.register_buffer("pos", self._make_pos_encoding(dim=dim, max_len=65))

    def _make_pos_encoding(self, dim: int, max_len: int) -> Tensor:
...
class FocalLoss:
    """
    Focal loss: weight positive examples more strongly
        so the model doesn't focus on the 99% negative examples

    Possible extensions:
        - a KL div term between time t and t+1
    """
    ...
    def __call__(self, input: Tensor, target: Tensor) -> Tensor:
        """
        input: (batch, time, freq)   logits
        target: (batch, time, freq)  one-hot
        """
...
if __name__ == "__main__":
    ...
    model = MusicTranscription(dim=96, n_heads=3, depth=4)
    ...
    # )
    # model.load_state_dict(torch.load("parameters\\model_big.pth"))
    test(
        model,
        val_loader,
        allowed_errors=[0, 1],
